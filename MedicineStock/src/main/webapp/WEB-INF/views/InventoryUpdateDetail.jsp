...
<div>
    <button style="float: right" id="add-btn" class="btn btn-primary" data-loading-text="Loading..."
            type="button">完成盘点
    </button>
...
<div class="jumbotron">
    ...
    <div align="center">
        <form id="frm" action="toinventorydetailbyinfo" method="post">
            ...
            <input type="hidden" name="inventoryMedicineVo.mark">
            <input type="hidden" name="inventoryVo.status" value="${inventory.status}"/>
            <input type="hidden" name="pageNo"/>
            <input type="hidden" id="geo" name="inventoryVo.geo" value="${inventory.id}"/>
    ...
    <table class="table table-hover">
        ...
        <s:iterator value="#inventoryMedicine" var="g">
            <tr class="tr">
                ...
                <td><input class="inventorystock form-control" style="display: inline-block;width: 130px" type="text" name="inventory.inventorystock"/></td>
                <td style="color: red"><span class="difference"></span>g/袋</td>
                <td><input type="text" class="form-control" style="display: inline-block;width: 130px" placeholder="备注"/></td>
            </tr>
        </s:iterator>
        </tbody>
    </table>
    ...
    <script>
        $(function () {
            ...
            $("#add-btn").click(function () {
                $(".tr").each(function (i) {
                    $.ajax({
                        url: 'updateinventorydetail',
                        type: 'post',
                        data:
                            {
                                "inventoryVo.geo":$("#geo").val(),
                                "inventoryMedicineVo.geo":$(this).find("td:eq(0)").html(), //指定给那些记录修改
                                "inventoryMedicineVo.inventorystock":$(this).find("td:eq(7)").find("input").val(),
                                "inventoryMedicineVo.difference":$(this).find("td:eq(8)").find("span").html(),
                                "inventoryMedicineVo.mark":$(this).find("td:eq(9)").find("input").val()
                            }
                    })
                });
                alert("提交成功！")
                //location.href='findinventory';
            })
            $(".inventorystock").blur(function () {
                $(this).parent().parent().find("td:eq(8)").find("span").html(parseFloat($(this).parent().parent().find("td:eq(7)").find("input").val())-parseFloat($(this).parent().parent().find("td:eq(6)").html()));
